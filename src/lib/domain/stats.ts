 ...
 */
const EFFECTIVE = "COALESCE(s.seen_on, to_char(s.created_at::timestamp, 'YYYY-MM-DD'))";

export async function profileStats(userId: number, viewerId: number | null) {
  ...
  const ratingHistogram = await all<{ rating: number; n: number }>(
  ...
  const byMonth = await all<{ month: string; n: number }>(
    `SELECT to_char(${EFFECTIVE}::timestamp, 'YYYY-MM') AS month, COUNT(*) AS n
       FROM sightings s
  ...
  const topTags = await all<{ tag: string; n: number }>(
  ...
  const mostRevisited = await all<{
    ...
    `SELECT w.slug, w.title, w.artist_display, COUNT(*) AS n
      ...
      WHERE s.user_id = ? ${privacy}
      GROUP BY w.id HAVING COUNT(*) > 1 ORDER BY n DESC LIMIT 6`,
    userId,
  );

  const venues = await all<{ name: string; slug: string; n: number }>(
 ...
 */
export async function yearInArt(userId: number, year: number) {
  ...
  const byMonth = await all<{ month: string; n: number }>(
    `SELECT to_char(${EFFECTIVE}::timestamp, 'MM') AS month, COUNT(*) AS n
       FROM sightings s
      WHERE s.user_id = ? AND ${EFFECTIVE} BETWEEN ? AND ?
  ...
  const newArtists = await all<{ artist_display: string; n: number }>(
      ...
      WHERE s.user_id = ? AND w.artist_display <> ''
        ...
        AND w.artist_display NOT IN (
          SELECT w2.artist_display FROM sightings s2 JOIN works w2 ON w2.id = s2.work_id
           WHERE s2.user_id = ? AND COALESCE(s2.seen_on, to_char(s2.created_at::timestamp, 'YYYY-MM-DD')) < ?
        )
  ...
  const busiestDay = await get<{ day: string; n: number }>(
...
export async function loggedYears(userId: number): Promise<number[]> {
  return (
    await all<{ y: string }>(
      `SELECT DISTINCT to_char(${EFFECTIVE}::timestamp, 'YYYY') AS y
