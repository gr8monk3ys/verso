...
export async function block(db, blockerId, blockedId) {
...
export async function unblock(db, blockerId, blockedId) {
  await db.prepare("DELETE FROM blocks WHERE blocker_id = ? AND blocked_id = ?").run(
    blockerId,
    blockedId,
  );
}
...
export async function hiddenUserIds(db, viewerId) {
  if (!viewerId) return [];
  const rows = await db
    .prepare(
...
export async function blockedByUser(db, blockerId) {
  return await db
    .prepare(
 ...
 */
export async function report(db, { reporterId, subjectType, subjectId, reason, note = "" }) {
...
export async function openReports(db, limit = 50) {
  return await db
    .prepare(
...
export async function resolveReport(db, reportId, staffId, status) {
  await db.prepare(
 ...
 */
export async function hideSighting(db, sightingId) {
  await db.prepare("UPDATE sightings SET review_public = 0 WHERE id = ?").run(sightingId);
}
