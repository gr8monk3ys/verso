...
test("nobody is staff by default", async () => {
  const db = await testDb();
  const id = await addUser(db, "priya");
  const row = await db.prepare("SELECT is_staff FROM users WHERE id = ?").get(id);
  assert.equal(row.is_staff, 0, "staff is granted by a person, never by signing up");
});

test("applying the schema twice is a no-op", async () => {
  // schema.sql is idempotent (CREATE ... IF NOT EXISTS throughout), so the
  // boot-time exec in db() is safe to run against an already-provisioned
  // database — the greenfield Postgres equivalent of the old migration guard.
  const db = await testDb();
...
test("a private diary's sightings stay out of other people's views", async () => {
  const db = await testDb();
  const shy = await addUser(db, "shy", { isPrivate: true });
  const row = await db.prepare("SELECT is_private FROM users WHERE id = ?").get(shy);
  assert.equal(row.is_private, 1);
});

test("a sighting on a private account is private however the sighting is flagged", async () => {
  ...
  const work = await addWork(db, "the-harvesters", { venueId: venue });
  ...
  const access = await sightingVisibility(db, sighting.id);
  assert.equal(access.ownerId, shy);
  assert.equal(access.isPrivate, true, "owner-private makes the sighting page owner-only");
});

test("sighting visibility: own flag, public case, and a missing row", async () => {
  const db = await testDb();
  const open = await addUser(db, "open");
  const work = await addWork(db, "vermeer");

  const flagged = await createSighting(db, { userId: open, workId: work, isPrivate: true });
  assert.equal((await sightingVisibility(db, flagged.id)).isPrivate, true);

  const publicOne = await createSighting(db, { userId: open, workId: work });
  assert.equal((await sightingVisibility(db, publicOne.id)).isPrivate, false);

  assert.equal(await sightingVisibility(db, 999999), null, "no row, no answer — the page 404s");
});

// ------------------------------------------------------- partial-form writes ---

test("rating from the queue leaves the privacy fields untouched", async () => {
  ...
  const work = await addWork(db, "rothko");
  const sighting = await createSighting(db, {
  ...
  await updateSighting(db, sighting.id, user, patch);
  const after = await db.prepare("SELECT * FROM sightings WHERE id = ?").get(sighting.id);
...
test("fields the form does post still write, including explicit clears", async () => {
  ...
  const work = await addWork(db, "hopper");
  const sighting = await createSighting(db, {
  ...
  await updateSighting(db, sighting.id, user, sightingPatchFromForm(form));
  const after = await db.prepare("SELECT * FROM sightings WHERE id = ?").get(sighting.id);
...
test("a partial-form patch never writes someone else's sighting", async () => {
  const db = await testDb();
  const owner = await addUser(db, "owner");
  const other = await addUser(db, "other");
  const work = await addWork(db, "goya");
  const sighting = await createSighting(db, { userId: owner, workId: work, rating: 10 });

  const form = new FormData();
  form.set("rating", "1");
  const result = await updateSighting(db, sighting.id, other, sightingPatchFromForm(form));
  assert.equal(result, undefined);
  assert.equal(
    (await db.prepare("SELECT rating FROM sightings WHERE id = ?").get(sighting.id)).rating,
    10,
  );
});
...
test("a replayed client_uuid belonging to someone else touches nothing", async () => {
  ...
  const db = await testDb();
  const victim = await addUser(db, "victim");
  const attacker = await addUser(db, "attacker");
  const venue = await addVenue(db, "met");
  const work = await addWork(db, "vermeer", { venueId: venue });

  const original = await createSighting(db, {
  ...
  const replay = await createSighting(db, {
  ...
  const after = await db.prepare("SELECT * FROM sightings WHERE id = ?").get(original.id);
...
test("the same user cannot hold one uuid twice, even bypassing the store", async () => {
  ...
  await insert.run("uuid-1", user, work);
  await assert.rejects(insert.run("uuid-1", user, work), /duplicate key|unique/i);
});

test("a replayed client_uuid of your own still carries a late rating", async () => {
  ...
  const first = await createSighting(db, {
    clientUuid: "own-uuid", userId: user, workId: work, venueId: venue,
    seenOn: "2026-07-01",
  });
  const second = await createSighting(db, {
    clientUuid: "own-uuid", userId: user, workId: work, venueId: venue,
